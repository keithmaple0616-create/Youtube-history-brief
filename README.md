...
视频生产工作台的目标是最终产出视频。主流程收敛为四步：

1. 分析完整正式文案。
2. 生成分镜头节奏，同时给出外部搜索素材清单和 image2 生图提示词。
3. 创作者回填搜集到的素材、source URL、license 和生成图片路径。
4. 调用 MiniMax TTS 生成配音，并导出完整 MP4。

过程文件会生成到被 Git 忽略的 `outputs/video-projects/`，不会污染源码目录。
